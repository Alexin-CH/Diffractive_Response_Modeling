...
def main(args, out_dir, out_path, zmax=1000):
    t0 = time.time()
    ...
    for _ in range(num_layers):
        ...
        idx = (z >= z_bot) & (z < z_top)
        # For the perm map, take the slice at x_idx
    ...
    sample['perm_map'] = perm_map.cpu()  # add permittivity map to the dict
    ...
    # Optionally also save the permittivity map separately
    pm_title = f"perm_map.A{int(amplitude)}_P{int(period)}_Z{int(zmax)}.pt"
    pm_path = os.path.join(out_dir, pm_title)
    torch.save(perm_map.cpu(), pm_path)
    print(f"Permittivity map saved to: {pm_path}")
...
if __name__ == "__main__":
    ...
    filename = (
        f"data_sim."
        f"{int(args.wl)}_{int(args.ang)}."
        f"{args.nh}_{args.discretization}."
        f"{int(args.sin_amplitude)}_{int(args.sin_period)}_{int(zmax)}.pt"
    )
    out_path = os.path.join(out_dir, filename)

    # Only run if the output doesn’t already exist
    if not os.path.exists(out_path):
        main(args, out_dir, out_path, zmax)
    else:
